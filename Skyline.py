    ...
    def recursive_function2(buildings: list) -> list:
        if len(buildings) == 1:
            return [BuildingTuple(buildings[0].height, buildings[0].left_coordinate),
                    BuildingTuple(0, buildings[
                        0].right_coordinate)]  # if the list has only one element, then its information and another fake building sharing the same right coordinate but a
        mid = len(buildings) // 2  # 0 left coordinate are used.
        left = Skyline.recursive_function2(
            buildings[:mid])  # method recursively called on the left and right side until the base case is found.
        right = Skyline.recursive_function2(buildings[mid:])

        return Skyline.merge(left,
                             right)  # method recursively used with the left and the right variables of the stored buildings
